...
static int meson_spicc_clk_init(struct meson_spicc_device *spicc)
{
	...
	parent_names[0] = clk_hw_get_name(&pow2_fixed_div->hw);
	init.num_parents = 1;
	...
	init.ops = &clk_divider_ops;
	init.flags = CLK_SET_RATE_PARENT;
	parent_names[0] = clk_hw_get_name(&enh_fixed_div->hw);
	init.num_parents = 1;
	...
	parent_names[0] = clk_hw_get_name(&pow2_div->hw);
	parent_names[1] = clk_hw_get_name(&enh_div->hw);
	init.num_parents = 2;
	init.flags = CLK_SET_RATE_PARENT;
